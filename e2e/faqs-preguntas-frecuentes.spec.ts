...
test.describe('FAQs - Preguntas Frecuentes', () => {
  ...
  test('should expand FAQ items when clicked', async ({ page }) => {
    const firstQuestion = page.getByRole("button", { name: "¿Qué es AeroTest?" })
    await expect(firstQuestion).toBeVisible()
    await firstQuestion.click()
    
    // Verificar que se expandió el contenido
    await expect(page.getByText(/AeroTest es una plataforma/)).toBeVisible()
  })
  ...
  test('should have questions under each category', async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "¿Qué es AeroTest?" })).toBeVisible()
